---
name: build-site
description: Build high-fidelity, cinematic "1:1 pixel-perfect" React landing pages from a fixed premium design system with 4 aesthetic presets (Organic Tech, Midnight Luxe, Brutalist Signal, Vapor Clinic), GSAP scroll animations, and a locked component architecture (floating navbar → hero → interactive feature cards → philosophy → sticky-stacking protocol → pricing → footer). Use this skill whenever the user wants to build a landing page, marketing site, hero page, product page, waitlist page, launch page, brand site, or any public-facing website — even if they don't say "cinematic" — and especially when they mention React, Vite, Tailwind, GSAP, or want something that feels premium, editorial, luxury, high-end, or "not generic AI slop". Trigger even when the user just says "build me a site for X" or "I need a landing page".
---
...
## Agent Flow — MINIMAL FRICTION, INFER FIRST

When the user asks to build a site, **do not run an intake questionnaire**. Taoufik has told Claude directly that he finds multi-question intakes annoying and trusts Claude to make judgment calls. The goal is to get from "build me a site for X" to a scaffolded, running Vite project with as few questions as possible — ideally zero.

### How to decide each parameter

1. **Brand name** — The only thing Claude genuinely cannot invent. If the user gave one in the prompt, use it. If not, pick a plausible one (2-3 syllables, evocative, domain-appropriate — e.g. "Meridian Partners" for recruitment, "Ember" for a coffee roaster, "Helix" for biotech). Present it in the announcement so the user can redirect with one line.

2. **One-line purpose** — Generate from the industry + any context in the prompt. Don't ask.

3. **Aesthetic preset** — Pick based on industry and tone. Do not ask. Default mapping:
   - Recruitment, finance, luxury, consultancy, law, watch/jewelry → **Midnight Luxe**
   - Biotech, wellness, longevity, organic/sustainable, clinical → **Organic Tech**
   - Dev tools, B2B SaaS, data platforms, productivity, "we broke X" disruptors → **Brutalist Signal**
   - Crypto, AI-forward, neon/cyberpunk, nightlife, gaming → **Vapor Clinic**
   - Anything ambiguous → **Midnight Luxe** (safest premium default)

4. **3 value propositions** — Generate plausible ones from the industry/brand purpose. Pick concrete, specific phrases (not vague marketing fluff). The user can swap them after seeing the draft; refactoring three strings is cheap.

5. **Primary CTA** — Industry default. Do not ask. Mapping:
   - Recruitment, consultancy, agency, coach → "Book a discovery call"
   - SaaS with free tier → "Start free trial"
   - SaaS without free tier, demo-driven → "Book a demo"
   - Waitlist / pre-launch product → "Join the waitlist"
   - E-commerce / D2C → "Shop the collection"
   - High-end service → "Request a consultation"

### Announce, then build

Before scaffolding, send the user a tight 4-5 line announcement:

```
Building **{brand}** — {one-line purpose}.
Aesthetic: **{preset}** ({why it fits in 4-6 words}).
Value props: {1} · {2} · {3}.
CTA: {cta}.
Scaffolding at {path}. Tell me now if you want a different brand name or aesthetic.
```

Then immediately start the build without waiting for a reply. The announcement is an opt-out, not a permission request — if the user is fine with the picks, they'll stay silent and see progress. If they want changes, they'll interrupt.

### When to break the rule

Only ask a real question (one, not four) if getting it wrong would cause *expensive rework* — e.g. the user is asking for something that materially changes the stack (Next.js vs Vite, SSR vs static), or they have a data source that needs to be wired up. Aesthetic and copy are not expensive rework; swapping a preset or rewriting three value props is a 2-minute edit.

### Tool usage during the build

- Use `TaskCreate`/TodoWrite to break the build into tracked steps (scaffold → fonts → hero → features → philosophy → protocol → pricing → footer → polish). Mark each done as you go so the user sees momentum.
- Use `Bash` for `npm create vite@latest`, installs, and the dev server.
- Use `Write` to create all source files in one pass, then `Edit` for refinements.
- Always `Read` before editing any existing file.
- Do not delegate the creative work to a subagent — the design sensibility must stay in the main loop. You may dispatch a subagent only for isolated asset research (e.g. picking Unsplash URLs that match a preset's `imageMood`).
...
6. Scaffold the project with `npm create vite@latest` in a new `websiteN` folder (if the user is inside `~/Taoufik - Websites/`, auto-increment from the highest existing `websiteN` / `Website N`; otherwise use a name derived from the brand). Install deps. Then `Write` every file in a single burst.
7. Ensure every animation is wired, every interaction works, every image loads. Run `npm run dev` in the background, verify the dev server starts cleanly, then **immediately auto-open the site in the default browser** using `open http://localhost:5173/` (macOS). Do not make the user click the URL themselves — Taoufik wants to see it instantly. If the port differs (Vite picks the next free port when 5173 is taken), parse the actual URL out of the dev server log before calling `open`. Only *after* the browser has been launched do you declare the build done.
